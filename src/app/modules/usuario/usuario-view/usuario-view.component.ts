...
import { DataDDMMYYYY } from 'src/app/shared/classes/util';
...
export class UsuarioViewComponent implements OnInit {
  ...
  constructor(
    ...
    this.formulario = formBuilder.group({
      ...
      grupo: [{ value: '' }, [Validators.required, Validators.min(1)]],
      grupo_: [{ value: '' }],
    ...
    this.usuario.cadastr = DataDDMMYYYY(new Date());
    this.grupos = [];
  ...
  ngOnInit() {
    if (this.idAcao == CadastroAcoes.Inclusao) {
      this.usuario = new UsuarioModel();
      this.usuario.id_empresa = this.globalService.getEmpresa().id;
      this.usuario.cadastr = DataDDMMYYYY(new Date());
      this.setValue();
    } else {
      this.getUsuario();
