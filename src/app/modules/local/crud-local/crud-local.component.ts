...
export class CrudLocalComponent implements OnInit {
  ...
  escolha(opcao: number, local?: LocalModel) {
    if (typeof local !== 'undefined') {
      let config = this.parametro.getParametro();
      Object(config).new = false;
      Object(config).id_retorno = local.id;
  ...
  getLocais() {
    ...
    this.inscricaoGetFiltro = this.localService
      ...
      .subscribe(
        (data: LocalModel[]) => {
          ...
          this.locais = [];
          this.locais = data;
          const idx = this.locais.findIndex(
            (cli) =>
              cli.id ==
