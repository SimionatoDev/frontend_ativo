...
export class CrudGrupoComponent implements OnInit {
  ...
  escolha(opcao: number, grupo?: GrupoModel) {
    if (typeof grupo !== 'undefined') {
      let config = this.parametro.getParametro();
      Object(config).new = false;
      Object(config).id_retorno = grupo.codigo;
  ...
  getGrupos() {
    ...
    this.inscricaoGetFiltro = this.grupoService
  ...
  loadParametros() {
    ...
    if (this.retorno && this.globalService.estadoFind('grupo') !== null) {
      const par = this.globalService.estadoFind('grupo');
      if (par != null) {
