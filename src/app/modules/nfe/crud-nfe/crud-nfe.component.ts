...
import { ParametroNfe01 } from 'src/app/parametros/parametro-nfe01';
...
@Component({
  selector: 'app-crud-nfe',
  templateUrl: './crud-nfe.component.html',
  styleUrls: ['./crud-nfe.component.css'],
})
export class CrudNfeComponent implements OnInit {
  ...
  constructor(
    ...
    this.inscricaoRota = route.params.subscribe((params: any) => {
      if (typeof params.retorno == 'undefined') {
        ...
        this.retorno = true;
        const par = this.globalService.estadoFind('nfe');
      }
    });
    this.loadParametros();
  }
  ...
  escolha(opcao: number, nfe?: NfeModel) {
    if (typeof nfe !== 'undefined') {
      let config = this.parametro.getParametro();
      Object(config).new = false;
      Object(config).id_retorno = nfe.nfe;
  ...
  getNfes() {
    let par = new ParametroNfe01();

    par.id_empresa = this.globalService.getEmpresa().id;

    par.id_filial = this.globalService.getLocal().id;

    if (this.parametros.value.campo == 'Nfe') {
      if (this.parametros.value.filtro != '') {
        par.nfe = this.parametros.value.filtro;
      }
    }
    ...
    this.inscricaoGetFiltro = this.nfeService
      .getNfesParametro_01(par)
      .subscribe(
        (data: NfeModel[]) => {
          this.globalService.setSpin(false);
          this.nfes = [];
          this.nfes = data;
          const idx = this.nfes.findIndex(
            (cli) =>
              cli.nfe ==
  ...
  loadParametros() {
    ...
    if (this.retorno && this.globalService.estadoFind('nfe') !== null) {
      const par = this.globalService.estadoFind('nfe');
      if (par != null) {
