...
@Component({
  ...
  templateUrl: './crud-principal.component.html',
  styleUrls: ['./crud-principal.component.css'],
})
export class CrudPrincipalComponent implements OnInit {
  ...
  escolha(opcao: number, principal?: PrincipalModel) {
    if (typeof principal !== 'undefined') {
      let config = this.parametro.getParametro();
      Object(config).new = false;
      Object(config).id_retorno = principal.codigo;
  ...
  getPrincipais() {
    ...
    this.inscricaoGetFiltro = this.principalService
      .getPrincipaisParametro_01(par)
      .subscribe(
        (data: PrincipalModel[]) => {
          this.globalService.setSpin(false);
          this.principais = [];
          this.principais = data;
          const idx = this.principais.findIndex(
            (cli) =>
              cli.codigo ==
        ...
        (error: any) => {
          ...
          this.appSnackBar.openFailureSnackBar(
            `Pesquisa Nos Principais ${messageError(error)}`,
            'OK'
          );
        }
      );
  ...
  loadParametros() {
    ...
    this.parametro.modulo = 'principal';
